...
function MainNav() {
  ...
  const renderMenuItem = (item) => {
    const isActive = item.href === '/' ? location.pathname === '/' : location.pathname.startsWith(item.href) && item.href !== '/';
    
    if (item.isCollapsible) {
      const isChildActive = item.subItems && item.subItems.some((subItem) => location.pathname === subItem.href);
      return (
        <Collapsible defaultOpen={isActive || isChildActive} className="group/collapsible">
          <CollapsibleTrigger asChild>
            ...
            <ChevronDown className={cn("h-4 w-4 ml-auto shrink-0 transition-transform duration-200 group-data-[state=open]/collapsible:rotate-180", sidebarState === 'collapsed' && 'hidden')} />
            </Button>
          </CollapsibleTrigger>
          {sidebarState === 'expanded' && item.subItems && item.subItems.length > 0 && (
                  ...
                  <li key={subItem.href}>
                    <Link to={subItem.href}
                      className={cn(
                        ...
                        location.pathname === subItem.href && "text-foreground font-semibold bg-primary/10"
                      )}>
                        {subItem.label}
                    </Link>
                  </li>
  ...
  return (
    <nav className="flex flex-col gap-1 px-2 py-4">
      ...
      {menuItems.map((item) => <div key={item.label}>{renderMenuItem(item)}</div>)}
      <div className="px-3 mt-4 mb-2 text-[10px] uppercase text-muted-foreground tracking-wider">Others</div>
      {otherItems.map((item) => <div key={item.label}>{renderMenuItem(item)}</div>)}
    </nav>
  );
}
      ...
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" className="relative flex items-center gap-2 h-10 px-2">
          <Avatar className="h-9 w-9">
            <AvatarImage src="https://i.pravatar.cc/150?u=musharof" alt="Musharof Chowdhury" />
      ...
      <DropdownMenuContent className="w-64" align="end" forceMount>
        <DropdownMenuLabel className="font-normal">
          <div className="flex flex-col space-y-1">
            <p className="text-sm font-medium leading-none">Musharof Chowdhury</p>
